...
const socialLinks = [
  ...
  {
    label: "GitHub",
    href: "https://github.com/purvravani0587",
    icon: (
      <svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24">
...
export default function Footer() {
  return (
    <footer className="bg-[#F8FAFC] text-slate-500">
      {/* Main Footer */}
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8 lg:gap-12">
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex flex-col gap-2 text-sm text-slate-600 mb-6">
              <div className="flex items-center gap-2">
                ...
                <span>fourstacktech02@gmail.com</span>
              </div>
              <div className="flex items-center gap-2">
                <Phone size={14} className="text-slate-600" />
                <span>+91 9537295884</span>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-3">
              {socialLinks.map(({ label, href, icon }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="w-9 h-9 rounded-lg bg-slate-100 hover:bg-slate-700 text-slate-500 hover:text-white flex items-center justify-center transition-all duration-200 hover:scale-110"
                >
                  {icon}
                </a>
              ))}
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              ...
              <div className="h-12 mb-5 hidden md:block" aria-hidden="true"></div>
              <h4 className="text-slate-900 text-sm font-semibold mb-4 uppercase tracking-widest">
                {category}
              </h4>
        ...
        <div className="mt-16 pt-8 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-slate-500 text-sm flex items-center gap-1.5">
            Built with
            <span className="text-slate-500 font-semibold"> Flutter </span>
            expertise &amp;
